  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: err.message || 'Please verify your parameters and try again.'
      });
    } finally {
    ...
    return (
      <div className="container mx-auto px-4 py-16 max-w-md space-y-8 animate-in fade-in duration-500">
        <div className="text-center space-y-3">
          ...
          <h1 className="text-3xl font-bold tracking-tight">Earn Per Submission</h1>
          <p className="text-muted-foreground text-sm">
            Please log in or register with email credentials to manage your digital balance wallet securely.
          </p>
        </div>
        ...
        <Card className="border shadow-lg bg-white rounded-xl overflow-hidden">
          <CardHeader className="pb-4">
            <CardTitle className="text-lg font-bold">
              {isRegistering ? 'Create an Account' : 'Sign In Gateway'}
            </CardTitle>
            <CardDescription>
              {isRegistering ? 'Register to start submitting digital items' : 'Access your submission monitor dashboard'}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleAuthSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                <Label htmlFor="authPassword">Secret Password</Label>
                <div className="relative">
            ...
            <CardContent className="pt-6">
              <form onSubmit={handleFormSubmit} className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="accountEmail">Target Email Address</Label>
                  <div className="relative">
                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                    <Input
                      id="accountEmail"
                ...
                <div className="space-y-2">
                  <Label htmlFor="password">Target Password</Label>
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                    <Input
                      id="password"
